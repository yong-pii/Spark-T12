    ...
    if(!sleep_fig)  //正常模式
    {
        ...
        if(i == 10)
        {
            ...
            else if(i == 85)
            {
                ...
                if(present_temp <= 450 )  //温度限制保护
                    PWMA_Duty.PWM2_Duty = PID(present_temp, expect_temp); //计算并输出占空比
                else
                    PWMA_Duty.PWM2_Duty = 0;

                if(present_temp < 180)  PWMA_Duty.PWM2_Duty = 58982; //低温下ADC无法正确读出数据，需要加热到一定温度。

                UpdatePwm(PWMA, &PWMA_Duty);             //PWM 占空比设置函数
            }
    }
    else if(sleep_fig)  //休眠模式
    {
        sleep_io_v = (3300 / 1024) * Get_ADCResult(10); //测sleep引脚电压
    }
}
...
void timer1_int (void) interrupt TIMER1_VECTOR
{
    ...
    if(sleep_io_v >= 28) sleep_time_s = 0; //检测到动作，休眠计时清零
