...
/*20v供电下稳定：Kp =16, Ti = 4, T = 8, Td = 1*/
h16 Kp = 18, Ti = 4, T = 8, Td = 1, Ek;
static h16 Ek_1, Ek_2 ;
u16 present_temp, expect_temp = 350; //当前温度，期望温度
...
/***********************************************************
*@fuction	:filter
*@brief		:滤波Num次，去掉一个最大值，去掉一个最小值，余下的求平均值
...
u16 filter(u8 Num)
{
    ...
    for(i = 0; i < Num; i++)
    {
        Vref = ( 1190 / Get_ADCResult(15) ) * 1024 ; //求出当前基准电压(Vref)
