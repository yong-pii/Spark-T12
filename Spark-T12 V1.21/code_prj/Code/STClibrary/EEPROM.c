...
u16 Start_addrs = 0x0000;
u8  xdata  eeprom_dat[10];  //power / tpme_skewing / temp/10 /sleep_time
...
{
    u8  xdata   dat[1];        //EEPROM操作缓冲
    EEPROM_read_n(address, dat, 1);
    return (dat[0]); //返回读取的值
}
/***********************************************************
*@fuction	:Eeprom_updata
*@brief		:更新EEPROM的数据
*@param		:start_address  起始地址
*@param		:*dat           写入数据源的缓冲的首地址
*@param		:Num            数据量
*@return	:void
*@author	:--xptx
*@date		:2023-02-25
...
void Eeprom_updata(u16 start_address, u8 *dat, u8 Num)
{
    u8 i, x = 0;
    u8  xdata   r_dat[10];        //EEPROM操作缓冲
    EA = 0;
    EEPROM_read_n(start_address, &r_dat, Num); //读出EEPROM的数据

    for(i = 0; i < Num; i++)
    {
        if(dat[Num] != r_dat[Num])
        {
            x++;
        }
    }
    if(x != 0)
    {
        EEPROM_SectorErase(start_address);  //擦除扇区
        EEPROM_write_n(start_address, dat, Num); //重新写入数据到扇区
...
/***********************************************************
*@fuction	:Eeprom_data_Init
*@brief		:数据初始化，读取EEPROM内的数据
*@param		:start_address  起始地址
*@param		:*dat           写入数据源的缓冲的首地址
*@param		:Num            数据量
*@return	:void
...
void Eeprom_data_Init(u16 start_address, u8 *dat, u8 Num)
{
    u8 tmp[10]; //临时缓存
    EA = 0;
    EEPROM_read_n(start_address,&tmp, Num); //读出EEPROM的数据

    if(tmp[0] < 5 || tmp[0] > 20) tmp[0] = 5;             // Power
    if(tmp[1] > 80) tmp[1] = 0;            // tpme_skewing
    if(tmp[2] < 20 || tmp[2] > 45) tmp[2] = 30; // temp/10
	
    if(tmp[3] < 5) tmp[3] = 5;             // sleep_time
  	if(tmp[3] > 30) tmp[3] = 15;             // sleep_time

    EEPROM_write_n(start_address, &tmp, Num); //重新写入数据到扇区
    EEPROM_read_n(start_address, dat, Num); //读出EEPROM的数据
