...
/*************************本工程使用了STC官方提供的标准外设驱动*****************************

一款基于增强型51内核的mcu STC8H1K17 控制的便携式T12电烙铁

由于是个人开发的，可能会有一些尚未发现的bug，所以将项目所有文件全部开源
欢迎各路大佬加入项目改bug~
...
bit IIC_communication_busy = 0;     //IIC通信总线状态标志位
u8  xdata   tmp[Max_Length];        //EEPROM操作缓冲
u16 xx, yy, zz;                     //陀螺仪3轴数据
u16 Ch1_vinput;                     //供电电压
u16 temp_T12, NTC_temp;             //热电偶电压，NTC电阻分压电压
/*******************************************************************/
...
extern u16  expect_temp;                  //当前温度，期望温度
/*******************************************************************/
...
void	PWM_config(void)
{
    ...
    PWMx_InitStructure.PWM_Period   = F_pwmti;							//周期时间,   PWM频率=Fosc/PWM_Period
...
void main(void)
{
    ...
    Eeprom_data_Init(Start_addrs, &eeprom_dat, 4); //数据初始化，读取EEPROM内的数据
    sync();  //同步数据
    power_Init();  //恢复供电电压记忆
    EA = 1;    //开中断
    OLED_Clear();  //oled清屏
